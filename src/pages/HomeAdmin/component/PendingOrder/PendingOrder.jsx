...
function PendingOrder() {
  ...
  const fetchOrderByUserId = async () => {
    try {
      const res = await axios.get(`/customorders/${orderId}`, {
  ...
  const cancelOrder = async () => {
    try {
      const res = await axios.put(`/customorders/${orderId}`, {
        params: { cancel: true, orderId },
      });
      const { data } = res;
      console.log(res);
    } catch (error) {
    ...
    try {
      const res = await axios.get("/customorders", {
        params: {
          sort,
          status,
          pages: `limit ${ordersPerPage} offset ${page * ordersPerPage}`,
        },
      });
      const { data } = res;
              ...
              <Grid item xs={3}>
                <FormControl>
                  ...
                  >
                    ...
                    <MenuItem key={2} value={"where status = 'waiting'"}>
                      Waiting
                    </MenuItem>
                    <MenuItem key={3} value={"where status = 'rejected'"}>
                      Rejected
                    </MenuItem>
                    <MenuItem key={4} value={"where status = 'approved'"}>
                      Approved
                    </MenuItem>
                  </Select>
                </FormControl>
              ...
              <Grid item xs={4}>
                <FormControl sx={{ m: 3, minWidth: 200 }}>
                  ...
                  >
                    ...
                    <MenuItem key={4} value={`order by created_at desc`}>
                      Latest
                    </MenuItem>
                    <MenuItem key={5} value={`order by created_at asc`}>
                      Oldest
                    </MenuItem>
                  </Select>
                </FormControl>
